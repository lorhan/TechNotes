...
From this discussion, it follows that the mapping of the $k$ terms in Equation \ref{eq_fourier_disc} to its corresponding frequency $f_{k}$ can be described by Equation \ref{eq_freq_resolution_2}:
...
In the current state of the setup, the vibration signals aquired by the sensors are transmitted via radio to the gateway devices, which then allows the user of the system to access and analyze the data through a desktop computer. With the improvements proposed in this work, however, the sensors should also perform signal processing tasks, so that they send over radio only the result of the processing instead of the complete signals, thus saving considerable battery energy.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.9\columnwidth]{graphics/pca_result/pca_result_v2.pdf}}
